...
- **同名の関数値は並行して実行できない：**
  ...
  - 異なる名前の関数値どうしは並行して進む。
- **所要時間：** 同期でも非同期でも、1回の呼び出しごとに最低1フレーム待つ。実測では、TurboWarp の既定の30fpsで約34.5ms、60fpsで約17.1ms、250fpsで約4.5msかかった（§4.8.5.1）。同名の関数値を100回 `start` すると、bridge モードでは逐次に処理されるので数秒かかる。compiled モードでは本当に並行して実行される。
...
**費用のモデル：** bridge モードでは、Promise を待つブロックは次のフレームまで再開できない。Promise の完了はマイクロタスクで伝わるため、VM がそのフレームの処理を終えるまで反映されないからである。この費用は関数値に限らず、**RPC を使う副作用ブロック（SQL、KV、外部 HTTP 通信、暗号など）すべて**にかかる。1回の待ちはちょうど1ステップである。

**実測値：**

- 計測環境：Node v26.7.0、TurboWarp scratch-vm 2.1.46、コンパイラ有効。
- 計測方法：ブラウザを使わない headless 実行で、「画面を再描画せずに実行」の独自ブロックの中でループさせた。
- 再現方法：`bench/bridge-latency.cjs`

| 実行方法 | 待ちなし（基準） | RPC（即時に完了） | RPC（I/O 1ms） | 関数値の `call` | 1回あたりのステップ数 |
|---|---|---|---|---|---|
| 30fps | ≈0 | 34.5ms | 34.5ms | 34.5ms | 1.0 |
| 60fps | ≈0 | 17.1ms | 17.1ms | 17.1ms | 1.0 |
| 250fps（TurboWarp の上限） | ≈0 | 4.5ms | 4.2ms | 4.5ms | 1.0 |
| 自分で `step` を呼ぶ（headless のテスト用ハーネス） | ≈0 | 0.016ms | 1.0ms（I/O が支配的） | 0.020ms | 1.0 |

ブラウザ上の TurboWarp エディタ（turbowarp.org/editor、Chromium 152。Claude アプリ内のブラウザ）でも、ページ内の `vm` に同じ計測用の拡張とプロジェクトを読み込んで計測した。

| 実行方法 | 待ちなし（基準） | RPC（即時に完了） | RPC（I/O 1ms） | 関数値の `call` | 1回あたりのステップ数 |
|---|---|---|---|---|---|
| 30fps | ≈0 | 33.6ms | 33.6ms | 33.6ms | 1.0 |
| 60fps | ≈0 | 16.1ms | 16.1ms | 16.1ms | 1.0 |
| 250fps | ≈0 | 4.0ms | **8.0ms** | 4.0ms | 1.0（I/O 1ms の RPC のみ2.0） |

- **Node の結果とほぼ一致した。** 関数値のスレッドは、ブラウザでもすべての試行で呼び出し元と同じステップの中で始まった。
- **I/O がフレームの間隔を超えると、その分だけフレームが増える。** ブラウザのタイマーは、1ms を指定しても実際にはそれより長くかかる。そのため、250fps（間隔4ms）では I/O 1ms の RPC に2フレームかかった。bridge モードでの待ち1回の費用は、正確には「フレームの間隔を単位として、I/O の時間を切り上げたもの」である。

**表示されていないタブでの計測（Chrome 153、macOS、2026-09-18）：** 利用者の Chrome で TurboWarp エディタを開いた。タブを表示したまま計測したあと、タブを裏に回した状態で約7分間、計測を繰り返した。

| 条件 | 30fps | 250fps |
|---|---|---|
| 表示中（中央値） | 39.7ms/回 ※ | 4.2ms/回 |
| 裏に回したタブ・通常のフレーム処理（`setInterval`） | **中央値 1200ms/回**（最大 21400ms/回） | **中央値 650ms/回**（600〜950ms/回） |
| 裏に回したタブ・Worker のタイマーでフレームを進める | 34.4ms/回（関数値の `call`：34.1ms/回） | 4.0ms/回（関数値の `call`：4.0ms/回） |

※ 30fps の表示中の値は、計測ループの中で毎回フレームレートを切り替えた分だけ上乗せされている（1回あたり1.2ステップ）。

この計測から分かったことは次の3つである。

- **裏に回したタブでは、フレームレートの設定に関係なく、待ち1回が約1秒になる。** Chrome が裏のタブのタイマーを最短1秒間隔に間引き、VM のフレーム処理（`setInterval`）もその影響を受けるからである。一般的な CRUD のハンドラ（待ち5回）で約6秒、ツールの呼び出し（待ち10回）で約12秒かかり、開発中の利用には耐えない。
- **それ以上に遅くなることもある。** 1回だけ、待ち1回あたり約21秒の遅延が出た。Chrome が長時間裏に回ったタブのタイマーをさらに強く間引く挙動（最短1分間隔）によるものと考えられるが、発生条件は特定できていない。
- **Worker のタイマーでフレームを進めると、裏に回したタブでも表示中と同じ速さになる。** VM 自身のフレーム処理を止め、Dedicated Worker の `setInterval` から届くメッセージを受けるたびに `runtime._step()` を呼ぶ方式である。Worker のタイマーは同じようには間引かれず、メインスレッドでメッセージを処理するのも遅れないため、この方式で間引きを避けられる。

実測から分かったことは次の3つである。

- **待ち1回の費用は、フレームの間隔そのものである。** I/O が1フレームより短ければ、I/O の時間は見えなくなる。
- **関数値の `call` の費用も、RPC と同じ1フレームである。** `startHats` で起動した関数値のスレッドは、すべての試行で呼び出し元と同じステップの中で始まった（30fpsでは60回中60回）。そのため、関数値の呼び出しは1回あたり1フレームで済み、2フレームにはならない。
- **自分で `step` を呼ぶハーネスでは、待ち1回が約0.02msになる。** これは30fpsの約1700分の1である。

以下の見積もりは、この実測値をもとにしている。
...
3. **bridge サーバにつながっている間は、Worker のタイマーでフレームを進める（P0）。** bridge 用の拡張は、接続中に VM 自身のフレーム処理（`frameLoop` の `setInterval`）を止め、Dedicated Worker の `setInterval` から届くメッセージを受けるたびに `runtime._step()` を呼ぶ。接続を切ったら元に戻す。
   - 実測では、裏に回したタブでの待ち1回が約1秒から表示中と同じ値に戻った（上の表）。対策がなければ、開発者がエディタを裏に回すだけで bridge モードは実用にならない。そのため P0 とする。
   - 注意点は次の3つである。
     - `frameLoop` の内部（`stop`、`framerate`、`currentStepTime`）に依存するので、TurboWarp の更新で壊れないよう、互換性テストに含める。
     - 画面の描画とその補間は、裏に回っている間は不要なので止めたままでよい。表示中は、元のフレーム処理と同じく描画も行う。
     - フレームレートの変更（`FRAMERATE_CHANGED`）を受けたら、Worker の間隔を変える。
4. **開発中はフレームレートを上げることを推奨する。** 開発手順書で、TurboWarp の詳細設定でフレームレートを上げる（例：250fps）ことを推奨する。bridge サーバへの接続時に、拡張がフレームレートが低いことを検出したら、編集画面に警告を出す。
5. **関数値ごとに、開発中もサーバで実行できるようにする（P2）。** `define function` に field `dev execution [vm/server]` を設ける。`server` を指定した関数値は、bridge モードでも VM を通さず、bridge サーバの IR インタプリタで実行する。デバッガの「ステップオーバー」にあたる。
...
- **確認済み（2026-09-18）：**
  - §4.8.5.1 の費用のモデル：待ち1回はちょうど1ステップだった。関数値のスレッドは呼び出し元と同じステップの中で始まった。
  - headless のハーネスで待ちの遅延がほぼなくなること：待ち1回あたり約0.02msだった。
  - TurboWarp の VM がコンパイラを有効にしたまま Node 上で動くこと：層3の PoC の前提も満たせる見込みである。
  - フレームレートの上限：250fps（`runtime.setFramerate` が250で頭打ちにする）。
  - 拡張から現在のフレームレートを取得する方法：`runtime.frameLoop.framerate` で取得でき、`FRAMERATE_CHANGED` イベントで変更を検知できる。0 は「画面のリフレッシュレート」を意味する。
  - ブラウザ上の TurboWarp エディタでも、Node とほぼ同じ値になること（§4.8.5.1）。
- **確認済み（2026-09-18）：** Chrome で裏に回したタブのタイマーの間引き。
  - 待ち1回が約1秒（最大で約21秒）になった。
  - Worker のタイマーでフレームを進める方式（§4.8.5.1 の対策3）で、表示中と同じ速さに戻ることを確認した。
  - Claude アプリ内のブラウザ（Electron）では、ページが非表示でも間引かれなかった。環境によって挙動が違うので、利用者が普段使うブラウザで確かめる必要がある。
- **未確認：** 対策3を長時間（例えば30分以上）続けた場合の挙動。Chrome が長時間裏に回ったタブをさらに強く間引く挙動が Worker に及ばないか、またタブの破棄（Tab Discarding）が起きないかを確かめる。Safari と Firefox での挙動も確かめる。
- **未確認：** 約21秒の遅延が出た条件（タブを裏に回してからの経過時間、ウィンドウの最小化や他のウィンドウによる遮蔽の違い）。
- JSON Schema のどのサブセットを使うかと、`eval` を使わない検証器の選定。
- Workers の外部リクエスト数と同時接続数の制限を確認し、§4.8.6 の同時実行数の上限（既定8）が妥当かを決める。
- bridge モードでの開発時の体感速度が、§4.8.5.1 の対策1〜4で許容できる範囲に収まるかを、参照アプリ（ログイン付き ToDo API、Webhook の受け口、Realtime エージェントのツール）で確認する。収まらなければ、対策5を P1 に繰り上げるか、クローンを使った並行実行か、振り分け用スクリプトの差し込みを再検討する。
